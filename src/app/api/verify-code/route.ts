...
/**
 * POST /api/verify-code  body: { collection, action: "create" | "update" | "delete", code }
...
export async function POST(req: Request) {
  try {
    ...
    if (!needsCode(collection, action)) return NextResponse.json({ ok: true, required: false });
    assertCode(collection, action, String(body.code ?? ""));
